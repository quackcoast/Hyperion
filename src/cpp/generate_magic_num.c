...
/* NOTE: I found this code from the chess programing wiki (chessprograming.org)
   as there is no easily found magic boards or numbers. you MUST generate them.
   This is a one time thing and the rook and bishop magic numbers are stored in
   bitboard.cpp. Compile information to see magic numbers is below.
*/
...
//
//    gcc generate_magic_num.c -o magic_finder -Wall -O2 -std=c11
//
//  ---------------------------
//
...
uint64 find_magic(int sq, int m, int bishop) {
  ...
  printf("***Failed to find magic for sq=%d, m=%d, bishop=%d after %d attempts***\n", sq, m, bishop, k);
  return 0ULL;
}
...
int main() {
  ...
  for(square = 0; square < 64; square++)
    printf("  0x%016llxULL, // sq %d\n", find_magic(square, RBits[square], 0), square);
  printf("};\n\n");

  printf("const uint64 BMagic[64] = {\n");
  for(square = 0; square < 64; square++)
    printf("  0x%016llxULL, // sq %d\n", find_magic(square, BBits[square], 1), square);
  printf("};\n\n");
